...
module.exports = {
      ...
      users: {
        post: {
          body: {
            ...
            capcha_code: joi.string().alphanum().length(4).required()
          },
          query: {
            capcha_key: joi.string().alphanum().length(16).required(),
            life: joi.number().integer().min(15 * 60).max(60 * 60 * 24 * 7).default(15 * 60) // second
          }
        }
      },
      admin: {
  ...
  methods: {
    ...
    token: {
      /**
       * @api {get} - check admin's token is right, need admin auth
       * @api {post} - get admin's auth token
       * @api {delete} - delete admin's auth token, need admin auth
       *
